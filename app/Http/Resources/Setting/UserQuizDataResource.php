    ...
    public function toArray($request)
    {
        ...
        return [
            'daily_attempt_corevalue'   => $this->collection->where('quiz_ID', '2')->first() ?
                (int) $setting[1]->value - $this->collection->where('quiz_ID', '2')->first()->attempt :
                (int) $setting[1]->value,
            'daily_attempt_dna'         => $this->collection->where('quiz_ID', '1')->first() ?
                (int) $setting[0]->value - $this->collection->where('quiz_ID', '1')->first()->attempt :
                (int) $setting[0]->value,
            'daily_attempt_collaborate' => $this->collection->where('quiz_ID', '3')->first() ?
                (int) $setting[2]->value - $this->collection->where('quiz_ID', '3')->first()->attempt :
                (int) $setting[2]->value,
        ];
    }
}
